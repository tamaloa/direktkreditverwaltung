...
class InterestCalculationTest < ActiveSupport::TestCase
  ...
  test "a list of interest rates with start and end should be returned" do
    # the interest calculation method is not relevant for this test, by default "30E_360" is used
    interest_calculation = InterestCalculation.new(@contract,
                                                   from: Date.new(2000),
                                                   till: Date.new(2020))
  ...
  test "only those interest rates should be returned which fall into the time interval" do
    # the interest calculation method is not relevant for this test, by default "30E_360" is used
    interest_calculation = InterestCalculation.new(@contract,
                                                   from: Date.new(2012, 7, 2),
                                                   till: Date.new(2012, 8, 2))

    assert_equal 1, interest_calculation.interest_rates_and_dates.count
    assert_equal 0.02.to_d,
                 interest_calculation.interest_rates_and_dates.first[:interest_rate]
  end
  ...
  test "account movements should always contain a initial balance entry even for far future dates" do
    ...
    interest_calculation = InterestCalculation.new(@contract,
                                                   from: Date.new(2200))
    assert_equal 1,
                 interest_calculation.account_movements_with_initial_balance.count
  end
  ...
  test "account movements should contain one entry for each movement plus initial balance" do
    ...
    interest_calculation = InterestCalculation.new(@contract,
                                                   from: Date.new(2000),
                                                   till: Date.new(2020))
    assert_equal (@contract.accounting_entries.count + 1),
                 interest_calculation.account_movements_with_initial_balance.count
  end
  ...
  test "interest_calculated_for_all_account_activities" do
    ...
    assert_equal expected_rows,
                 interest_calculation.interest_calculated_for_all_account_activities.count
  end

  test "interest total should be calculated correctly for calculation-method '30E_360'" do
    interest_calculation = InterestCalculation.new(@contract, 
                                                   from: Date.new(2012, 1, 1), 
                                                   till: Date.new(2012, 12, 31), 
                                                   method: "30E_360")
    assert_equal 78.0.to_s, interest_calculation.interest_total.to_s
    assert_equal 78.0.to_s,
                 InterestCalculation.new(@contract, year: 2012, method: "30E_360").interest_total.to_s
  end

  test "interest total should be calculated correctly for calculation-method 'act_act'" do
    contract = Contract.find_by_number("01-04-001")

    interest_calculation = InterestCalculation.new(contract, 
                                                   from: Date.new(2014, 3, 4),
                                                   till: Date.new(2014, 12, 31),
                                                   method: "act_act")
    assert_equal 24.82.to_s, interest_calculation.interest_total.to_s
    #assert_equal 78.0.to_s, InterestCalculation.new(contract, year: 2015, method: "act_act").interest_total.to_s
  end
